  ...
  const label =
    kind === "read+problem"
      ...
      : kind === "solve"
        ...
        : "READ";
  return (
    <span className="mono rounded bg-surface-variant px-2 py-0.5 text-[11px] text-on-surface-variant">
      {label}
    </span>
  );
}
    ...
    >
      ...
      >
        <div className="flex flex-col gap-1">
          <h2 className="display text-xl font-semibold tracking-tight text-on-surface sm:text-2xl">
            {section.title}
          </h2>
        ...
        <div className="flex items-center gap-6">
          ...
          <span
            className={`material-symbols-outlined text-on-surface-variant transition-transform duration-300 ease-out ${
              open ? "rotate-180" : ""
            }`}
          >
            expand_more
      ...
      <div className="accordion-panel" data-open={open ? "true" : "false"}>
        <div className="accordion-panel-inner">
          <div
            className={`flex flex-col border-t border-outline-variant/20 ${
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <span className="mono w-8 text-xs text-on-surface-variant">
                        {lesson.order}
                      </span>
                      <span className="text-sm font-medium text-on-surface transition-colors group-hover/row:text-primary">
                        {lesson.title}
                      </span>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <span className="flex items-center gap-2 rounded p-2 text-on-surface-variant transition-colors group-hover/row:bg-primary/10 group-hover/row:text-primary">
                      <span className="hidden text-sm font-medium group-hover/row:inline">
                        Open lesson
                      </span>
...
function ModuleBlock({
  ...
  return (
    ...
    >
      <div
        className={`absolute bottom-0 left-0 top-0 bg-primary transition-all duration-300 group-hover:w-2 ${
          open ? "w-1 opacity-100" : "w-0 opacity-0"
        }`}
      />
      <button
        type="button"
        onClick={onToggle}
        className={`flex w-full items-center justify-between p-6 text-left transition-colors duration-300 ${
          open ? "bg-surface-container-highest/20" : ""
        }`}
      >
        <div className="flex flex-col gap-1">
          <h2 className="display text-xl font-semibold tracking-tight text-on-surface sm:text-2xl">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <Link
                    to={`/problems/${p.id}`}
                    tabIndex={open ? 0 : -1}
                    className="text-sm font-medium text-on-surface transition-colors group-hover/row:text-primary"
                  >
                    {p.title}
                  </Link>
...
export default function DSASheet() {
  ...
  const patternModule = useMemo(() => {
    const filters = { difficulty, status, topic };
    const problems = patternProblems.filter((p) => matchesFilters(p, filters));
  ...
  }, [patternProblems, difficulty, status, topic]);
      ...
      <main className="mx-auto mt-16 flex w-full max-w-container-max flex-1 gap-gutter px-margin-mobile py-8 md:px-margin-desktop">
        ...
        <aside className="hidden w-64 shrink-0 flex-col gap-6 lg:flex">
          <div className="relative flex flex-col items-center gap-4 overflow-hidden rounded-xl border border-outline-variant/30 bg-surface-container-low p-6 text-center">
            ...
            {user ? (
              <>
                ...
                <div>
                  <h3 className="mb-1 text-sm font-medium text-on-surface">
                    {user.name}
                  </h3>
              ...
              <>
                ...
                <Link
                  to="/login"
                  className="w-full rounded border border-primary px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-primary/10"
                >
                  Sign In
                </Link>
              </>
          ...
          <div className="flex flex-col gap-6 rounded-xl border border-outline-variant/30 bg-surface-container-low p-6">
            <h3 className="verdict-strip text-on-surface-variant">Filters</h3>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-8">
          <div className="flex flex-col gap-2">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div className="flex flex-col gap-2">
                <span className="verdict-strip text-primary">
                  DSA Sheet / Learn
                </span>
                <h1 className="display text-3xl font-semibold tracking-tight text-on-surface sm:text-4xl">
                  Learn by solving
                </h1>
          ...
          {stats && (
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low px-4 py-2">
                ...
                <span className="mono text-sm text-on-surface">
                  {stats.total}
                </span>
              </div>
              <div className="flex items-center gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low px-4 py-2">
                ...
                <span className="mono text-sm text-easy">
                  {stats.solved}
                </span>
              </div>
              <div className="flex items-center gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low px-4 py-2">
                ...
                <span className="mono text-sm text-medium">
                  {stats.attempted}
                </span>
              </div>
              <div className="flex items-center gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low px-4 py-2">
                ...
                <span className="mono text-sm text-on-surface-variant">
                  {stats.notStarted}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low p-6">
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-6">
                    <div className="flex flex-col items-end gap-1">
                      <span
                        className={`mono text-xs ${
                          patternModule.solved > 0
                            ? "text-primary"
                            : "text-on-surface-variant"
                        }`}
                      >
                        {patternModule.solved}/{patternModule.total} (
                        {patternModule.percent}%)
                      </span>
                      <div className="h-1 w-32 overflow-hidden rounded-full bg-surface-variant">
                        <div
                          className="h-full bg-primary transition-all duration-300"
                          style={{ width: `${patternModule.percent}%` }}
                        />
                      </div>
                    </div>
                    <span
                ...
                <div
                  ...
                  data-open={patternsOpen ? "true" : "false"}
                >
                  <div className="accordion-panel-inner">
                    <div
                      className={`mt-4 flex flex-col ${
                        patternsOpen ? "" : "pointer-events-none"
                      }`}
                      aria-hidden={!patternsOpen}
                    >
                      {patternModule.problems.length === 0 ? (
                        <p className="text-sm text-on-surface-variant">
                          No pattern problems loaded.
                        </p>
                      ) : (
                        patternModule.problems.map((p, idx, list) => (
                          <div
                            key={p.id}
                            className={`group/row flex items-center justify-between px-1 py-4 transition-colors hover:bg-surface-variant/30 ${
                              idx < list.length - 1
                                ? "border-b border-outline-variant/10"
                                : ""
                            }`}
                          >
                            <div className="flex min-w-0 items-center gap-4">
                              {statusIcon(p.status)}
                              <span className="mono w-8 shrink-0 text-xs text-on-surface-variant">
                                {idx + 1}
                              </span>
                              <Link
                                to={`/problems/${p.id}`}
                                tabIndex={patternsOpen ? 0 : -1}
                                className="truncate text-sm font-medium text-on-surface transition-colors group-hover/row:text-primary"
                              >
                                {p.title}
                              </Link>
                              <div className="hidden flex-wrap gap-2 sm:flex">
                                {p.topics
                                  .filter((t) => t !== "Pattern Problems")
                                  .slice(0, 2)
                                  .map((t) => (
                                    <span
                                      key={t}
